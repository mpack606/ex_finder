...
pub fn view(state: &TabsState) -> Element<'static, TabsMessage> {
    ...
    for (i, nav) in state.list.iter().enumerate() {
        ...
        .on_press(TabsMessage::Close(i));

        let tab_btn = button(
            row![text(folder_name).size(13).width(Length::Fill), close_btn]
                .spacing(8)
